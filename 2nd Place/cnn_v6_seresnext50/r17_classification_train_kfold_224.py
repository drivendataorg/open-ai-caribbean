...
MODELS_PATH_KERAS = MODELS_PATH + 'classification_{}_224/'.format(BACKBONE)
...
def strong_aug(p=1.0):
    return Compose([
        ...
        ShiftScaleRotate(shift_limit=0.1, scale_limit=0.1, rotate_limit=45, p=0.5, border_mode=cv2.BORDER_REFLECT),
        OneOf([
...
def mask_aug(p=1.0):
    return Compose([
        ShiftScaleRotate(shift_limit=0.05, scale_limit=0.1, rotate_limit=10, p=1.0, border_mode=cv2.BORDER_REFLECT),
    ],  p=p)
...
def randomly_change_mask(mask):
    mod_mask = mask_aug()(image=mask)['image']
    return mod_mask
...
def augment_image(orig_img):
    ...
    img = orig_img[:, :, :3]
    mask = orig_img[:, :, 3]
    mask = randomly_change_mask(mask)
    ...
    start_0 = random.randint(delta_start, delta_end)
    end_0 = random.randint(img.shape[0] - delta_end, img.shape[0] - delta_start)
    start_1 = random.randint(delta_start, delta_end)
    end_1 = random.randint(img.shape[1] - delta_end, img.shape[1] - delta_start)
...
def model_gen(size, upd_ch):
    from keras.models import Model, load_model
    from keras.layers import Dense, Dropout, Input, GlobalAveragePooling2D

    backbone = BACKBONE
    ref_ch = 3
    required_layer_bn_name = 'bn_data'
    required_layer_conv_name = 'conv2d_1'

    weights_cache_path = CACHE_PATH + 'model_{}_{}ch_imagenet_{}.h5'.format(backbone, upd_ch, size)
    if not os.path.isfile(weights_cache_path):
        classifier, _ = Classifiers.get(backbone)
        model_ref = classifier(include_top=False, weights='imagenet', input_shape=(size, size, ref_ch))
        print(model_ref.summary())

        classifier, _ = Classifiers.get(backbone)
        model_upd = classifier(include_top=False, weights=None, input_shape=(size, size, upd_ch))

        for i, layer in enumerate(model_ref.layers):
            print('Update weights layer [{}]: {}'.format(i, layer.name))
            if layer.name == required_layer_conv_name:
                print('Recalc CONV weights!')
                ...
                continue
            elif layer.name == required_layer_bn_name:
                print('Recalc BN weights!')
                bn_config = layer.get_config()

                if bn_config['scale']:
                    gamma, beta, run_mean, run_std = layer.get_weights()
                else:
                    gamma = 1.0
                    beta, run_mean, run_std = layer.get_weights()

                print(gamma, beta, run_mean, run_std)
                beta = np.array(list(beta) + [beta[0]])
                run_mean = np.array(list(run_mean) + [run_mean[0]])
                run_std = np.array(list(run_std) + [run_std[0]])
                w_new = (beta, run_mean, run_std)
                print(w_new)
        ...
        print('Restore from cache: {}'.format(weights_cache_path))
...
def create_keras_model(fold_number):
    ...
    train_ids_all, train_answ_all, valid_ids_all, valid_answ_all = get_kfold_split(KFOLD_NUMBER, seed=26)
    ...
    model_name = '{}_fold_{}'.format(BACKBONE, fold_number)
    optim_name = 'Adam'
    batch_size_train = 32
    batch_size_valid = 32
...
Ep 26: 185/185 [==============================] - 53s 289ms/step - loss: 0.4183 - acc: 0.8435 - val_loss: 0.4867 - val_acc: 0.8142
Ep 22: 185/185 [==============================] - 58s 313ms/step - loss: 0.4620 - acc: 0.8261 - val_loss: 0.4912 - val_acc: 0.8118
Ep 28: 186/186 [==============================] - 54s 292ms/step - loss: 0.4245 - acc: 0.8383 - val_loss: 0.4965 - val_acc: 0.8156
'''