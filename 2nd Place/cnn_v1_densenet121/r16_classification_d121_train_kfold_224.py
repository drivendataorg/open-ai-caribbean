...
MODELS_PATH_KERAS = MODELS_PATH + 'classification_densenet121_v2_224_kfold/'
...
def strong_aug(p=1.0):
    return Compose([
        ...
        IAAAffine(scale=1.0, translate_percent=(-0.1, 0.1), translate_px=None, rotate=0.0, shear=(-10, 10),
                  order=1, cval=0, mode='reflect', always_apply=False, p=0.01),
...
def model_DenseNet121_multich(size, upd_ch):
    ...
    weights_cache_path = CACHE_PATH + 'model_DenseNet121_{}ch_imagenet_{}.h5'.format(upd_ch, size)
...
def create_keras_model(fold_number):
    ...
    model_name = 'densenet121_v2_fold_{}'.format(fold_number)
    optim_name = 'Adam'
    batch_size_train = 24
    batch_size_valid = 24
