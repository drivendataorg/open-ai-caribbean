...
from cnn_v2_irv2.r17_classification_irv2_train_kfold_299_v2 import *
...
def get_validation_score(models_paths):
    ...
    for fold_number, model_path in enumerate(models_paths):
        ...
        model = model_IRV2_multich_v2(299, 4)
    ...
    subm_path = FEATURES_PATH + 'irv2_kfold_v2_valid_TTA_32_{}.csv'.format(len(models_paths))
...
def proc_tst(models_paths, score):
    ...
    for i, c in enumerate(CLASSES):
        d[c] = overall_preds[:, i]
    subm_path = FEATURES_PATH + 'irv2_kfold_v2_test_TTA_32_{}.csv'.format(len(models_paths))
    d.to_csv(subm_path, index=False)

    shutil.copy2(subm_path, SUBM_PATH + 'irv2_dense_v2_test_TTA_32_{}_loss_{}.csv'.format(len(models_paths), score))
...
'''
TTA 32
Fold vrfd: 0 Accuracy: 0.835736 Log loss: 0.430418
Fold vrfd: 1 Accuracy: 0.833893 Log loss: 0.431860
Fold vrfd: 2 Accuracy: 0.831204 Log loss: 0.454208
Fold vrfd: 3 Accuracy: 0.822439 Log loss: 0.459315
Fold vrfd: 4 Accuracy: 0.835413 Log loss: 0.446995
Only verified validation stat Loss: 0.444538 Acc: 0.831742 LB: 0.4211

TTA 168:
Fold vrfd: 0 Accuracy: 0.836071 Log loss: 0.427723
Fold vrfd: 1 Accuracy: 0.833893 Log loss: 0.429217
Fold vrfd: 2 Accuracy: 0.831876 Log loss: 0.448410
Fold vrfd: 3 Accuracy: 0.827493 Log loss: 0.455232
Fold vrfd: 4 Accuracy: 0.835076 Log loss: 0.442739
Only verified validation stat Loss: 0.440644 Acc: 0.832885
'''
