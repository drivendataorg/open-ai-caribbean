...
from cnn_v2_irv2.r16_classification_irv2_train_kfold_299 import *
...
def get_validation_score(models_paths):
    ...
    for fold_number, model_path in enumerate(models_paths):
        ...
        model = model_IRV2_multich(299, 4)
    ...
    subm_path = FEATURES_PATH + 'irv2_kfold_valid_TTA_32_{}.csv'.format(len(models_paths))
...
def proc_tst(models_paths, score):
    ...
    for i, c in enumerate(CLASSES):
        d[c] = overall_preds[:, i]
    subm_path = FEATURES_PATH + 'irv2_kfold_test_TTA_32_{}.csv'.format(len(models_paths))
    d.to_csv(subm_path, index=False)

    shutil.copy2(subm_path, SUBM_PATH + 'irv2_kfold_test_TTA_32_{}_loss_{}.csv'.format(len(models_paths), score))
...
'''
TTA 32
Fold vrfd: 0 Accuracy: 0.834395 Log loss: 0.425185
Fold vrfd: 1 Accuracy: 0.831544 Log loss: 0.442274
Fold vrfd: 2 Accuracy: 0.835575 Log loss: 0.448866
Fold vrfd: 3 Accuracy: 0.818059 Log loss: 0.463613
Fold vrfd: 4 Accuracy: 0.825295 Log loss: 0.474320
Only verified validation stat Loss: 0.450813 Acc: 0.828985 LB: 0.4256
'''
