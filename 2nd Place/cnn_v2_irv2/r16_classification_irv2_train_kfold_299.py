...
KFOLD_NUMBER = 5
FOLD_LIST = [0, 1, 2, 3, 4]
SHAPE_SIZE = (299, 299, 3)
MODELS_PATH_KERAS = MODELS_PATH + 'classification_irv2_299_kfold/'
...
def model_IRV2_multich(size, upd_ch):
    from keras.models import Model, load_model
    from keras.layers import Dense, Dropout, Input
    from keras.applications import InceptionResNetV2

    ref_ch = 3
    required_layer_name = 'conv2d_1'

    weights_cache_path = CACHE_PATH + 'model_IRV2_{}ch_imagenet_{}.h5'.format(upd_ch, size)
    if not os.path.isfile(weights_cache_path):
        model_ref = InceptionResNetV2(include_top=False,
                          weights='imagenet',
                          input_shape=(size, size, ref_ch),
                          pooling='avg',)
        model_upd = InceptionResNetV2(include_top=False,
                                   weights=None,
...
def create_keras_model(fold_number):
    ...
    model_name = 'irv2_v2_fold_{}'.format(fold_number)
    optim_name = 'Adam'
    batch_size_train = 16
    batch_size_valid = 16
    ...
    callbacks = [
        ...
        ReduceLROnPlateau(monitor='val_loss', factor=0.9, patience=5, min_lr=1e-9, epsilon=0.00001, verbose=1),
