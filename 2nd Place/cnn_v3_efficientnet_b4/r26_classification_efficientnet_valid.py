...
from cnn_v3_efficientnet_b4.r17_classification_efficientnet_train_kfold_380 import *
...
def get_validation_score(models_paths):
    ...
    subm_path = FEATURES_PATH + 'effnetb4_kfold_valid_TTA_32_{}.csv'.format(len(models_paths))
...
def proc_tst(models_paths, score):
    ...
    for i, c in enumerate(CLASSES):
        d[c] = overall_preds[:, i]
    subm_path = FEATURES_PATH + 'effnetb4_kfold_test_TTA_32_{}.csv'.format(len(models_paths))
    d.to_csv(subm_path, index=False)

    shutil.copy2(subm_path, SUBM_PATH + 'effnetb4_kfold_test_TTA_32_{}_loss_{}.csv'.format(len(models_paths), score))
...
'''
TTA 32
Fold vrfd: 0 Accuracy: 0.841770 Log loss: 0.419143
Fold vrfd: 1 Accuracy: 0.841275 Log loss: 0.412894
Fold vrfd: 2 Accuracy: 0.838937 Log loss: 0.435703
Fold vrfd: 3 Accuracy: 0.836253 Log loss: 0.440250
Fold vrfd: 4 Accuracy: 0.831366 Log loss: 0.449819
Only verified validation stat Loss: 0.431532 Acc: 0.837929 LB: 0.4082

TTA 168
Fold vrfd: 0 Accuracy: 0.842105 Log loss: 0.417542
Fold vrfd: 1 Accuracy: 0.841275 Log loss: 0.406864
Fold vrfd: 2 Accuracy: 0.838937 Log loss: 0.431760
Fold vrfd: 3 Accuracy: 0.837938 Log loss: 0.433893
Fold vrfd: 4 Accuracy: 0.832715 Log loss: 0.445715
Only verified validation stat Loss: 0.427127 Acc: 0.838601
'''
