...
from cnn_v4_densenet169.r17_classification_densenet169_train_kfold_224 import *
...
def get_validation_score(models_paths):
    ...
    subm_path = FEATURES_PATH + 'densenet169_kfold_valid_TTA_32_{}.csv'.format(len(models_paths))
...
def proc_tst(models_paths, score):
    ...
    for i, c in enumerate(CLASSES):
        d[c] = overall_preds[:, i]
    subm_path = FEATURES_PATH + 'densenet169_kfold_test_TTA_32_{}.csv'.format(len(models_paths))
    d.to_csv(subm_path, index=False)

    shutil.copy2(subm_path, SUBM_PATH + 'densenet169_kfold_test_TTA_32_{}_loss_{}.csv'.format(len(models_paths), score))
...
'''
TTA 32
Fold vrfd: 0 Accuracy: 0.836882 Log loss: 0.426434
Fold vrfd: 1 Accuracy: 0.827364 Log loss: 0.434456
Fold vrfd: 2 Accuracy: 0.830242 Log loss: 0.437698
Fold vrfd: 3 Accuracy: 0.846992 Log loss: 0.414795
Fold vrfd: 4 Accuracy: 0.839806 Log loss: 0.429491
Fold vrfd: 5 Accuracy: 0.843129 Log loss: 0.426389
Only verified validation stat Loss: 0.428215 Acc: 0.837391 LB: 
'''
