...
# Model
args.update({
    'basemodel_name': 'inceptionv4',
    'pretrained': 'imagenet+background',  # 'imagenet' or 'imagenet+background' or 'imagenet+5k'
...
def get_dataloaders(path_to_data, datasets, args=ARGS):
    ...
    ## Data reader classes
    roof_data_reader = {
        'train': RoofDataset(datasets['train'], data_dict, buffer=1, mask_image=False, plot_polygons=True),
        'valid': RoofDataset(datasets['valid'], data_dict, buffer=1, mask_image=False, plot_polygons=True),
        'fold': RoofDataset(datasets['fold'], data_dict, buffer=1, mask_image=False, plot_polygons=True),
        'test': RoofDataset(datasets['test'], data_dict, buffer=1, mask_image=False, plot_polygons=True),
    }
